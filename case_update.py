    ...
    def scrapping(self):
        ...
        for id in id_list:
            ...
            try:
                original_TCID = driver.find_element_by_class_name(
                    'customfield_10202').text
                GM_REQ_ID = driver.find_element_by_class_name(
                    'customfield_10324').text
                summary = driver.find_element_by_class_name(
                    'summary').text
                objective = driver.find_element_by_class_name(
                    'customfield_10336').text
                precondition = driver.find_element_by_class_name(
                ...
                expected = driver.find_element_by_class_name(
                    'customfield_10315').text
                Frop_List_1 = driver.find_element_by_class_name(
                    'customfield_10200').text
                Frop_List_2 = driver.find_element_by_class_name(
                    'customfield_10319').text
                Story_ID = driver.find_element_by_class_name(
                    'customfield_10601').text
                Tag1 = driver.find_element_by_class_name(
                    'customfield_10374').text
                print('Found!')
                print('==========================================')
                Date = ''
                Assignee = 'feiliu'
                case_detail = [Date, original_TCID, Assignee, GM_REQ_ID, summary, objective, precondition, test_step, expected, Frop_List_1, Frop_List_2, Story_ID, Tag1]
                self.wb['Detailed list'].append(case_detail)
            except:
                print('cannot find the detail of case: {}'.format(id))
                print('==========================================')
...
scrp = Scraper('search_list.xlsx', 'CaseUpdate.xlsx')
